/*************************************************************************
	...
	> Created Time: Thu 31 Aug 2017 09:28:01 AM CST
 ************************************************************************/
...
public:
    ...
    void Put(K key, T data)
    {
        ...
        {
            if(free_entries_.empty()) //可用节点为空，即cache已满
            {
                node = tail_->prev;
                detach(node);
    ...
    T Get(K key)
    {
        ...
        else    //如果cache中没有，返回T的默认值 这与hashmap行为一致
        {
            return T();
        }
    }
...
private:
    ...
    vector<Node<K, T>* > free_entries_;  //储存可用节点的地址
    Node<K, T> *head_, *tail_;
    Node<K, T> *entries_;           //双向链表的节点
};
